...
reverseWords("Hello world from JS");
...
function logElementsWithIndex(arr) {
  for (let i=0;i<arr.length;i++) console.log(`Index: ${i}, Value: ${arr[i]}`);
}
...
function findMin(numbers) {
  let min = numbers[0];
  for (let num of numbers) if (num < min) min = num;
  console.log(min);
}
findMin([5,2,9,1]);
...
function countOccurrences(arr, word) {
  let count = 0;
  for (let item of arr) if (item === word) count++;
  console.log(count);
}
...
function removeFalsyValues(arr) {
  let result = [];
  for (let item of arr) if (item) result.push(item);
  console.log(result);
}
removeFalsyValues([0,1,"",2,null,3,false,4]);
...
function sumDigits(str) {
  let sum = 0;
  for (let ch of str) if (!isNaN(ch) && ch !== " ") sum += Number(ch);
  console.log(sum);
}
sumDigits("abc123"); 
...
function flattenArray(twoDArray) {
  ...
  for (let arr of twoDArray) {
    for (let item of arr) result.push(item);
  }
  console.log(result);
}
...
function findWordsWithLetter(words, letter) {
  let result = [];
  for (let word of words) if (word.includes(letter)) result.push(word);
  console.log(result);
}
