...
console.log("===== CONTACT BOOK TEST =====");

/* Display contacts */
displayAllContacts(contacts);

/* Add new contacts */
addContact("Seda", "440-1111", "seda@gmail.com", contacts);
addContact("Furkan", "440-2222", "furkan@gmail.com", contacts);
addContact("Balamir", "440-3333", "balamir@gmail.com", contacts);

/* Try adding duplicate contact */
addContact("Sada", "555-9999", "sada2@gmail.com", contacts);

/* Display contacts again */
displayAllContacts(contacts);

/* View a contact */
viewContact("Seda", contacts);
viewContact("Balamir", contacts);

/* Update a contact */
updateContact("Furkan", "440-7777", "furkan_new@gmail.com", contacts);

/* View updated contact */
viewContact("Furkan", contacts);

/* Remove a contact */
removeContact("Balamir", contacts);

/* Try removing non-existing contact */
removeContact("Tufan", contacts);

/* Search contacts */
searchContacts("se", contacts);
...
1. Partial Name Search:
   ...
   function searchContacts(keyword, contactBook) {
  console.log("Searching contacts for:", keyword);

  let foundAny = false;

  for (let i = 0; i < contactBook.length; i++) {
    if (contactBook[i].name.toLowerCase().includes(keyword.toLowerCase())) {
      console.log(contactBook[i].name, contactBook[i].phone, contactBook[i].email);
      foundAny = true;
    }
  }
...
/*2. Sort Contacts:
  ...
  console.log("Sorting contacts alphabetically by name...");

  // sort fonksiyonu kullanıyoruz
  contactBook.sort(function(a, b) {
    // isimleri küçük harfe çevirip karşılaştırıyoruz
  ...
  displayAllContacts(contactBook); // sıralanmış listeyi göster
}
...
*/
  ...
  for (let i = 0; i < contactBook.length; i++) {
    ...
    // name, phone veya email içinde keyword var mı?
    if (
  ...
  if (!foundAny) {
    console.log("No contacts found for:", keyword);
  }

searchContactByPhoneOrEmail("xyz", contacts); 