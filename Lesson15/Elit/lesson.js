...
checkProtocol("https.elitALTUN.com");
...
function repeatWord(word, times) {
  const repeatedResult = word.repeat(5);
...
censorWord("It is a secret password", "password");
...
function sliceLastN(text, n) {
  const slicedWord = text.slice(-2);
  console.log(slicedWord);
}

sliceLastN("Javascript");
...
replaceCharacter("kikikikiki", "i", "o");
...
titleCase("the weather is very hot");
...
isLongString("javascript is becoming fun");
isLongString("fun");
...
isSpam("ali veli spam");
isSpam("ali veli deli");
...
function getInitials(fullName) {
  const nameParts = fullName.split(" "); // ['elit', 'altun']
...
containsNumber("elitaltun1994");
containsNumber("elitaltun");
...
function reverseWords(sentence) {
  ...
  const reversedWordsArray = words.map((word) => {
    return word.split("").reverse().join(" ");
  });
  const result = reversedWordsArray.join(" ");
  console.log(result);
}
reverseWords("elit altun");
...
findWordPosition("elit altun yalova merkez", "yalova");
findWordPosition("elit altun yalova merkez", "istanbul");
