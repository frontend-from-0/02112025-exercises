...
addContact('Ayşe', '555-0000', 'ayse@mail.com', contacts);

updateContact('Guner', '999-9999', 'guner_new@mail.com', contacts);

removeContact('muhammed', contacts);

removeContact('Hakan', contacts);
...
*/
function searchByName(query, contactBook) {
  console.log(`Searching for: "${query}"...`);
  
  // filter() metodu, şarta uyan tüm elemanlardan yeni bir dizi oluşturur
  ...
  if (results.length > 0) {
    ...
    displayAllContacts(results); // Hazırladığın fonksiyonu burada tekrar kullanabilirsin
  } else {
...
2. Sort Contacts:
  ...
  // localeCompare, farklı dillerdeki karakterleri (ö, ş, i gibi) doğru sıralamak için en güvenli yoldur.
  contactBook.sort((a, b) => a.name.localeCompare(b.name));
  
  console.log("Contacts sorted alphabetically.");
  displayAllContacts(contactBook);
}
/*
3. Search by multiple fields:
   - e.g., find a contact by phone number or email.
*/
function universalSearch(searchTerm, contactBook) {
  ...
  if (results.length > 0) {
    ...
    console.log("No matches found in any field.");
  }
}